...
//! the Arrow IPC random-access file format. The Kotlin original uses the JVM
//! Arrow library's `ArrowFileWriter` / `ArrowFileReader` for the same format,
//! so the on-disk byte layout is identical and the two implementations are
//! wire-compatible.
//!
...
impl ShuffleManager {
    ...
    /// Kotlin: `writePartition(jobUuid, stageId, partitionId, batches)`.
    ///
    /// Matches the Kotlin original's no-op-on-empty behaviour. The partition
    /// directory is created if it doesn't exist. All batches share the
    /// schema of `batches[0]` (Kotlin makes the same assumption).
    pub fn write_partition(
        ...
        create_dir_all(&dir).unwrap_or_else(|e| {
            panic!(
                "failed to create shuffle directory {}: {e}",
                dir.display()
            )
        });
        ...
        let file = File::create(&file_path).unwrap_or_else(|e| {
            panic!(
                "failed to create shuffle file {}: {e}",
                file_path.display()
            )
        });

        // The schema is taken from the first batch (Kotlin's `batches.first().schema`).
        let schema = batches[0].schema();
        let mut writer = FileWriter::try_new(file, &schema)
            .expect("failed to initialise Arrow IPC FileWriter");
        ...
        writer
            .finish()
            .expect("failed to finalise Arrow IPC file");
    }

    /// Read a partition's batches from local Arrow IPC storage.
    /// Kotlin: `readPartition(jobUuid, stageId, partitionId)`.
    ///
...
#[cfg(test)]
mod tests {
    //! `ShuffleManager` has no Kotlin test — kquery only exercises it
    //! end-to-end via the Flight executor. We add a standalone round-trip test
    //! here because the manager is fully self-contained and worth proving
    ...
    fn write_then_read_round_trips_batches() {
        ...
        mgr.write_partition(job_uuid, stage_id, partition_id, &[batch1.clone(), batch2.clone()]);
