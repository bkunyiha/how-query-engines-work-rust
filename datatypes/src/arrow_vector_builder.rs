//! Port of `kquery/datatypes/src/main/kotlin/ArrowVectorBuilder.kt`.
//!
//! Builds an arrow-rs `ArrayRef` value by value, then wraps it in an
//! [`ArrowFieldVector`]. The Kotlin original wraps a mutable Java Arrow
//! `FieldVector` and exposes `set(i, value)` plus `setValueCount(n)`. arrow-rs
//! uses an *append*-based builder pattern instead of indexed mutation: typed
//! builders like `Int32Builder::new()` accumulate values, then `.finish()`
//! seals them into an immutable `ArrayRef`.
//!
//! Translation notes (the most significant API divergence in module 1):
//! - **Kotlin `set(i, value)` → Rust `append_value(value)` / `append_null()`.**
//!   Indexed mutation is *not* the arrow-rs way; the column is built strictly
//!   in row order, which is also how every kquery call site uses the builder
//!   in practice (loops `for i in 0..n { b.set(i, ...) }` with sequential
//!   `i`). For the few call sites that wanted to skip an index, we model that
//!   as `append_null()`.
//! - **Kotlin `setValueCount(n)` → not needed.** arrow-rs builders track this
//!   automatically as `.len()` and `.finish()` writes it into the output array.
//!   We expose a no-op `set_value_count(_n)` so a literal port of any call
//!   site still compiles, but it does nothing in Rust.
//! - **Kotlin's runtime type-dispatch (10+ `is X -> ...` arms over
//!   `FieldVector`)** is preserved via an enum [`ArrowVectorBuilder`] with
//!   one variant per supported builder type. The Kotlin `set` accepts a
//!   nullable `Any?`; the Rust port accepts a [`ScalarValue`] which the
//!   builder dispatches against its own variant.
//! - **`build()` returns [`ArrowFieldVector`]** wrapping the finished
//!   `ArrayRef`. Same shape as Kotlin's `build()` returning a `ColumnVector`.
//! - **Decimal / Decimal256 not yet supported.** kquery's `Decimal128Vector`
//!   handling depends on Java's `BigDecimal` arithmetic. The Rust equivalents
//!   are `Decimal128Array` / `Decimal256Array` with precision + scale in the
...
impl ArrowVectorBuilder {
    ...
    /// row capacity. Panics on unsupported types (matches Kotlin's
    /// `throw IllegalStateException` for unknown vector types).
    ...
    ///
    /// The Kotlin equivalent is `set(i: Int, value: Any?)`. We accept a
    /// `ScalarValue` because the typed enum carries both the value and its
    /// type — equivalent to Kotlin's runtime type-checked dispatch but
    /// resolved at compile time.
    ///
    /// Panics if the value's type doesn't match the builder's type, matching
    /// the Kotlin original's `throw IllegalStateException()` for mismatched
    /// types.
    ...
    /// Append a null. Equivalent to Kotlin's `fieldVector.setNull(i)`.
    ...
    /// No-op shim for the Kotlin `setValueCount(n)` call. arrow-rs builders
    /// track length automatically; this method exists so a literal port of
    /// any Kotlin call site still compiles.
    ...
    /// finished `ArrayRef`. Matches Kotlin's `build(): ColumnVector`.
...
mod tests {
    ...
    /// Port of `ColumnVectorTest.kt`'s `build int vector` test. The Kotlin
    /// test constructs an `IntVector`, allocates room for 10, populates it
    /// 0..10 via `ArrowVectorBuilder.set(it, it)`, builds the
    /// `ArrowFieldVector`, and asserts size==10 and each value matches its
    /// index. The Rust shape is the same with `append_value` instead of `set`.
    #[test]
    fn build_int_vector() {
