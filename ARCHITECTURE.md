...
| Both `datatypes/ShuffleId.kt` and `datatypes/ShuffleLocation.kt` are unreachable code in upstream kquery; the live `ShuffleLocation` is a different, 6-field definition in `physical-plan/ShuffleLocation.kt` | `grep -rn "ShuffleId\b\|ShuffleIdAction\|ShuffleLocation\b" how-query-engines-work/ --include="*.kt"` | `datatypes/ShuffleId` → referenced only by `physical-plan/Action.kt`'s `data class ShuffleIdAction(val shuffleId: ShuffleId)`, which is itself never constructed (`ProtobufDeserializer.fromProto(Action)` only builds `QueryAction`; `else` throws `NotImplementedError`). `datatypes/ShuffleLocation` (4 fields: `jobUuid, stageId, partitionId, executionUuid`) → referenced by nothing. `physical/ShuffleLocation` (6 fields: adds `executorId, executorHost, executorPort`) → referenced by `distributed/`, `flight-server/`, `examples/`, `protobuf/`, `SchedulerTest.kt`. **Faithful-port disposition:** all three files (`datatypes/shuffle_id.rs`, `datatypes/shuffle_location.rs`, and the future `physical-plan/shuffle_location.rs`) are ported as-is, mirroring kquery exactly; pruning dead code is a redesign decision reserved for a future Rustified rewrite. | §4.1 (datatypes), §4.6 (physical-plan), TRANSLATION_NOTES.md under module `datatypes` |
